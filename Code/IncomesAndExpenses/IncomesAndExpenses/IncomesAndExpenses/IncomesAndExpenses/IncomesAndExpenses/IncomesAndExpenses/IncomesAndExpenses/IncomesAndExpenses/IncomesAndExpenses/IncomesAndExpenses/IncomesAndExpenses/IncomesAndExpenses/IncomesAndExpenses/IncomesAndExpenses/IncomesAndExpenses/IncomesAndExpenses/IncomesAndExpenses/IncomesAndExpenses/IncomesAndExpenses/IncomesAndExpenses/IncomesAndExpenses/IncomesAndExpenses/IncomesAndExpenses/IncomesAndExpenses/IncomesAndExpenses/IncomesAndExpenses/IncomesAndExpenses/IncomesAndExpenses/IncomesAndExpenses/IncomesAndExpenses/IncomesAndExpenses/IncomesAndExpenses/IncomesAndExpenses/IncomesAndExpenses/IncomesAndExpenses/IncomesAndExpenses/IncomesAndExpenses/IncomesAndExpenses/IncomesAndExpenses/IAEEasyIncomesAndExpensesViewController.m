...
- (UIView *)withoutConceptsWarningView
{
    if (!_withoutConceptsWarningView) {
        _withoutConceptsWarningView = [UIView viewFromXib:kXibWithoutConceptsWarningViewName withOwner:self.editAndReportModeContentContainerView];
        _withoutConceptsWarningView.center = self.editAndReportModeContentContainerView.center;
    }
...
- (void)setConceptsCollectionViewInTransitionAspect:(BOOL)transition withLogicBlockAfterFinish:(void(^)(void))logicBlock
{
    [UIView animateWithDuration:kDurationOfEditConceptCollectionViewTransition animations:^{
        if (transition) {
            ...
            self.withoutConceptsWarningView.alpha = 0.0;
        } else {
